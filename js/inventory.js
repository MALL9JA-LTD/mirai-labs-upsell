...
async function loadAll() {
  try {
    const [products, staff, agentInv, dispatches, deliveries] = await Promise.all([
      ...
      fetchAll((from, to) =>
        window._supabase.from('deliveries').select('id,status,product_id,quantity,items,delivery_staff_id').order('id').range(from, to)
      ),
    ]);
...
function populateDropdowns() {
  ...
  staffSel.innerHTML = '<option value="">— Select staff —</option>' + allStaff.map(s => `<option value="${s.id}">${s.name}</option>`).join('');
  const prodSel = document.getElementById('dispatch-product');
  prodSel.innerHTML = '<option value="">— Select product —</option>' + allProducts.map(p => `<option value="${p.id}">${p.name} (Stock: ${p.total_stock||0})</option>`).join('');
}
  ...
  allDeliveries.forEach(d => {
    ...
    items.forEach(it => {
      if (it.product_id === p.id) {
        ...
        if (d.status === 'delivered') unitsDelivered += Number(it.qty||1);
      }
    });
  });
  const pending = unitsOrdered - unitsDelivered;
  const revenue = allDeliveries.filter(d => d.status==='delivered' && (
    (Array.isArray(d.items) && d.items.some(it => it.product_id===p.id)) || d.product_id===p.id
  )).reduce((s,d) => s + Number(d.sale_price||0), 0);
  return { unitsOrdered, unitsDelivered, pending, revenue };
}
  ...
  grid.innerHTML = allProducts.map(p => {
    ...
    return `<div class="product-card">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4px;">
        ...
        <button class="btn-outline btn-sm" onclick="editProduct('${p.id}')">Edit Product</button>
      </div>
      <div class="product-stats">
        <div class="product-stat"><label>Selling Price</label><span>${fmtMoney(p.selling_price)}</span></div>
        <div class="product-stat"><label>Cost per Unit</label><span>${fmtMoney(p.cost_price)}</span></div>
        <div class="product-stat"><label>Total Stock</label><span>${p.total_stock||0}</span></div>
      </div>
      <div class="product-stats">
        <div class="product-stat"><label>Units Ordered</label><span>${stats.unitsOrdered}</span></div>
        <div class="product-stat"><label>Units Delivered</label><span>${stats.unitsDelivered}</span></div>
        <div class="product-stat"><label>Pending Delivery</label><span>${stats.pending}</span></div>
      </div>
      <div class="product-stat" style="margin-top:4px;"><label>Revenue (delivered)</label><span style="font-size:16px;">${fmtMoney(stats.revenue)}</span></div>
    </div>`;
  }).join('');
}
...
function renderStockTable() {
  ...
  // Header: STAFF + one col per product
  thead.innerHTML = '<tr><th>STAFF</th>' + allProducts.map(p => `<th>${p.name}</th>`).join('') + '</tr>';
  tbody.innerHTML = allStaff.map(s => {
    const productCells = allProducts.map(p => {
    ...
    return `<tr><td><strong>${s.name}</strong>${s.active ? '' : ' <span style="color:var(--ml-muted);font-size:11px;">(inactive)</span>'}</td>${productCells}</tr>`;
  }).join('');
}
...
async function dispatchInventory(e) {
  ...
  try {
    ...
    const { data: upd, error: updErr } = await window._supabase.from('products').update({ dispatched_stock: newDispatched }).eq('id',productId).select();
...
async function saveDispatch() {
  ...
  btn.disabled=true; btn.textContent='Saving…';
  const { data, error } = await window._supabase.from('inventory_dispatches').update({ quantity: qty, notes }).eq('id',id).select();
