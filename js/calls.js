  ...
  const queries = [
    fetchAll((from, to) => {
      let q = window._supabase.from('customers')
        .select('id,full_name,phone,state,order_date,original_product,assigned_to')
...
function populateAgentFilter() {
  ...
  sel.innerHTML = '<option value="">All Agents</option>' +
    allProfiles.map(p => `<option value="${p.id}">${p.full_name}</option>`).join('');
}

function renderCallRate() {
  const chipsEl = document.getElementById('call-rate-chips');
  if (!chipsEl) return;
  const today = new Date().toISOString().split('T')[0];
  chipsEl.innerHTML = allProfiles.filter(p => p.role === 'crs_agent').map(p => {
    ...
    return `<div class="call-rate-chip">${p.full_name} <span>${todayCalls} today · ${totalCalls} total</span></div>`;
  }).join('');
}
...
function applyQueueFilter() {
  ...
  // Sort: Tier A first, then B, then C, then by last call date asc (least recently called first)
  const tierOrder = { A: 0, B: 1, C: 2 };
  filteredQueue.sort((a, b) => {
    const ta = tierOrder[calcTier(a.order_date)] ?? 2;
    const tb = tierOrder[calcTier(b.order_date)] ?? 2;
    if (ta !== tb) return ta - tb;
    const callsA = allCallLogs.filter(c => c.customer_id === a.id);
    const callsB = allCallLogs.filter(c => c.customer_id === b.id);
    const lastA  = callsA.length ? new Date(callsA[0].call_date) : new Date(0);
    const lastB  = callsB.length ? new Date(callsB[0].call_date) : new Date(0);
    return lastA - lastB; // least recently called first
  });
...
function renderQueueList() {
  ...
  list.innerHTML = filteredQueue.map(c => {
    ...
    return `<div class="queue-item${isSelected ? ' selected' : ''}" data-id="${c.id}">
      ...
      <div style="display:flex;gap:4px;flex-wrap:wrap;">${tierBadge(tier)}${statusBadge(lastOutcome)}</div>
    </div>`;
  }).join('');
...
function renderRecordPanel(c) {
  ...
  panel.innerHTML = `
    ...
    <div style="font-size:9px;text-transform:uppercase;letter-spacing:0.2em;color:var(--ml-gold-dim);margin-bottom:12px;">Call History (${calls.length})</div>
    ${calls.length === 0
      ...
      : calls.map(cl => `
        <div class="call-history-item">
          ...
          <button class="btn-ghost btn-sm" onclick="openEditCall('${cl.id}')">Edit</button>
        </div>`).join('')
    }`;
}
...
function openCallModal(prefillCustomer) {
  ...
  document.getElementById('call-modal-title').textContent = selectedCustomer
    ? `Log Call — ${selectedCustomer.full_name}`
    : 'Log New Call';
  itemRows = [];
...
function selectModalCustomer(c) {
  ...
  document.getElementById('customer-list').style.display = 'none';
  document.getElementById('selected-customer-info').innerHTML =
    `${c.phone || ''} | ${c.state || ''} | ${tierBadge(calcTier(c.order_date))}`;
...
async function submitCall() {
  ...
  try {
    if (editId) {
      const { error } = await window._supabase.from('call_logs')
        .update({ outcome, channel, notes }).eq('id', editId).select();
    ...
    if (outcome === 'ordered') {
      ...
      if (validItems.length === 0) { showToast('Please add at least one product item', 'error'); btn.disabled = false; btn.textContent = 'Save Call'; return; }
      ...
      const { data: existing } = await window._supabase.from('deliveries')
        .select('id').eq('customer_id', customerId).eq('status', 'pending').limit(1);
      if (existing && existing.length > 0) {
        const { data: upd, error } = await window._supabase.from('deliveries')
          .update(delivPayload).eq('id', existing[0].id).select();
        if (error) throw error;
...
function bindEvents() {
  ...
  document.getElementById('btn-log-call-activity').addEventListener('click', () => {
